...
def show_main():
    ...
    window = sg.Window(
        "QuickdataLoad - Main", layout=layout, icon=icon_image, margins=(10, 10)
    )
            ...
            while True:
                ...
                if event == "Execute":
                    ...
                    try:
                        ...
                        if (
                            validators.length(mapping_file, 1)
...
def transform(mapping_file, main_sheet, input_file, output_file, win):
    ...
    parallelize_tranformation(
        mapping_file, main_sheet, input_data, output_file, n_cores=1
    )
...
def open_about():
    ...
    """
    sg.popup_ok(
        "QuickdataLoad, Version: {}".format(__version__), about_text, icon=icon_image
    )
